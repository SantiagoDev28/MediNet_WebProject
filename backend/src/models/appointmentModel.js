...
class Appointment {
    ...
    static async create(citaData) {
        try {
            ...
            const disponibilidad = await this.checkAvailability(citaData.medico_id, citaData.cita_fecha, citaData.cita_hora);
            if (!disponibilidad) {
                throw new Error('El médico no está disponible en esa fecha y hora');
            }

            const query = `
                INSERT INTO citas (medico_id, paciente_id, cita_fecha, cita_hora, cita_estado) 
                VALUES (?, ?, ?, ?, ?)
            `;
            ...
            await this.updateAvailability(citaData.medico_id, citaData.cita_fecha, citaData.cita_hora, 0);
    ...
    static async update(cita_id, citaData) {
        try {
            ...
            if (citaData.cita_fecha !== citaActual.cita_fecha || citaData.cita_hora !== citaActual.cita_hora) {
                const disponibilidad = await this.checkAvailability(citaActual.medico_id, citaData.cita_fecha, citaData.cita_hora);
                if (!disponibilidad) {
                    throw new Error('El médico no está disponible en esa nueva fecha y hora');
                }

                // Liberar el horario anterior
                await this.updateAvailability(citaActual.medico_id, citaActual.cita_fecha, citaActual.cita_hora, 1);
                
                // Ocupar el nuevo horario
                await this.updateAvailability(citaActual.medico_id, citaData.cita_fecha, citaData.cita_hora, 0);
            }
    ...
    static async delete(cita_id) {
        try {
            ...
            if (cita) {
                await this.updateAvailability(cita.medico_id, cita.cita_fecha, cita.cita_hora, 1);
            }
    ...
    static async updateAvailability(medico_id, fecha, hora, estado) {
        try {
            const query = `
                ...
                WHERE medico_id = ? AND disponibilidad_fecha = ? AND disponibilidad_hora = ?
            `;
            await db.executeQuery(query, [estado, medico_id, fecha, hora]);
        } catch (error) {
    ...
    static async getStats() {
        try {
            ...
            return {
                ...
                citas_hoy: results[1][0][0].citas_hoy,
